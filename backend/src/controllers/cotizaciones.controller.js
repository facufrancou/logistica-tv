...
// NOTA: Reserva automática DESHABILITADA - Los lotes se asignan solo por orden del usuario
async function asignarStockFIFO(idVacuna, cantidadRequerida, fechaAplicacion, tx = prisma) {
  // ============================================================
  // RESERVA AUTOMÁTICA DESHABILITADA
  // Los lotes se asignarán manualmente desde el calendario
  // usando las funciones: asignarLoteManual, asignarMultiplesLotes
  // ============================================================
  console.log(`[RESERVA DESHABILITADA] Vacuna ${idVacuna}: Se requieren ${cantidadRequerida} dosis para ${fechaAplicacion}. Asignar manualmente.`);
  return null;

  /* CÓDIGO DE RESERVA AUTOMÁTICA COMENTADO:
  try {
...
exports.createCotizacion = async (req, res) => {
  try {
    ...
    // ============================================================
    // RESERVAS AUTOMÁTICAS DESHABILITADAS
    // Los lotes se asignarán manualmente desde el calendario
    // ============================================================
    console.log('[RESERVA DESHABILITADA] Cotización creada sin reservas automáticas. Asignar lotes manualmente desde el calendario.');

    /* CÓDIGO DE RESERVAS AUTOMÁTICAS COMENTADO:
...
exports.eliminarCotizacion = async (req, res) => {
  try {
    ...
    // PRIMERO: Liberar lotes asignados en el calendario de vacunación
    const itemsConLote = await prisma.calendarioVacunacion.findMany({
      where: { 
        id_cotizacion: parseInt(id),
        id_stock_vacuna: { not: null }
      },
      include: { stock_vacuna: true }
    });

    if (itemsConLote.length > 0) {
      console.log(`Liberando ${itemsConLote.length} lotes asignados en calendario por eliminación...`);
      
      // Agrupar por lote para optimizar updates
      const lotesALiberar = new Map();
      for (const item of itemsConLote) {
        const key = item.id_stock_vacuna;
        if (lotesALiberar.has(key)) {
          lotesALiberar.get(key).cantidad += item.cantidad_dosis;
        } else {
          lotesALiberar.set(key, {
      ...
      // Liberar cada lote
      for (const [idStock, info] of lotesALiberar) {
        await prisma.stockVacuna.update({
          where: { id_stock_vacuna: idStock },
          data: {
            stock_reservado: { decrement: info.cantidad }
          }
        });
        ...
        await prisma.movimientoStockVacuna.create({
          data: {
            ...
            motivo: 'Liberación por eliminación de cotización',
            observaciones: `Cotización eliminada: ${cotizacion.numero_cotizacion}`,
            id_cotizacion: parseInt(id),
            id_usuario: idUsuario
          }
        });

        console.log(`Liberadas ${info.cantidad} dosis del lote ${info.lote}`);
      }

      // Limpiar asignaciones en calendario
      ...
      console.log(`Liberados ${lotesALiberar.size} lotes diferentes del calendario`);
    }

    // SEGUNDO: Liberar reservas antiguas (tabla reservaStockVacuna)
...
exports.marcarEntregaDosis = async (req, res) => {
  try {
    ...
    const { 
      ...
      lotes_utilizados // NUEVO: array opcional con lotes y cantidades específicas
    } = req.body;
    ...
    // Verificar que el calendario exists
    const calendarioExistente = await prisma.calendarioVacunacion.findUnique({
      where: { id_calendario: parseInt(id_calendario) },
      include: {
        producto: true,
    ...
    // NUEVA LÓGICA: Detectar si hay múltiples lotes asignados
    let lotesParaDescontar = [];
    
    if (lotes_utilizados && Array.isArray(lotes_utilizados) && lotes_utilizados.length > 0) {
    ...
    if (calendarioExistente.producto.requiere_control_stock) {
      ...
      if (calendarioExistente.cotizacion.id_plan && lotesParaDescontar.length > 0) {
        ...
        if (totalReservado < cantidad_entregada) {
          return res.status(400).json({ 
            error: 'Stock reservado insuficiente en los lotes asignados',
            total_reservado: totalReservado,
            solicitado: cantidad_entregada,
            lotes: lotesParaDescontar.map(l => l.lote).join(', '),
            sugerencia: 'Reasigne lotes en el calendario antes de registrar la entrega'
          });
        }
      } else if (!calendarioExistente.cotizacion.id_plan) {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (calendarioExistente.producto.requiere_control_stock) {
        // Si es un plan de vacunación y tiene lotes asignados
        if (calendarioExistente.cotizacion.id_plan && lotesParaDescontar.length > 0) {
          console.log(`🚚 Procesando entrega de ${cantidad_entregada} dosis desde ${lotesParaDescontar.length} lote(s)`);
          
          // NUEVA LÓGICA: Descontar proporcionalmente de cada lote según su reserva
          let dosisRestantes = cantidad_entregada;
          
          for (const loteInfo of lotesParaDescontar) {
            if (dosisRestantes <= 0) break;
            
            // Obtener stock actualizado del lote
            const stockLote = await tx.stockVacuna.findUnique({
              where: { id_stock_vacuna: loteInfo.id_stock_vacuna }
            });
            
            if (!stockLote) {
              console.warn(`⚠️ Lote ${loteInfo.lote} no encontrado, saltando...`);
              continue;
            }
            ...
            console.log(`  Cantidad a descontar: ${cantidadADescontar} dosis`);
            ...
            await tx.movimientoStockVacuna.create({
              data: {
                ...
                observaciones: lotesParaDescontar.length > 1 
                  ? `Entrega múltiple lotes: ${cantidadADescontar}/${cantidad_entregada} dosis - ${responsableCompleto}`
                  : `Entrega: ${cantidadADescontar} dosis - ${responsableCompleto}`,
            ...
            console.log(`  ✅ Lote ${stockLote.lote} actualizado. Dosis restantes: ${dosisRestantes}`);
          }
...
exports.generarRemitoPDF = async (req, res) => {
  try {
    ...
    // NUEVA LÓGICA: Detectar múltiples lotes y obtener cantidades de movimientos
    let lotesEntregados = [];
    
    // Buscar los movimientos de stock de esta entrega para obtener cantidades exactas
    const movimientosEntrega = await prisma.movimientoStockVacuna.findMany({
      ...
      take: 10 // Últimos 10 movimientos para esta entrega
    });

    if (movimientosEntrega.length > 0) {
      // Hay movimientos registrados, usar esos datos
      ...
      console.log('⚠️ No se encontraron movimientos, usando fallback');
      
      if (calendario.observaciones && calendario.observaciones.includes('Lotes múltiples asignados:')) {
        ...
        if (match) {
          ...
          // Distribuir proporcionalmente entre los lotes
          const cantidadPorLote = Math.floor(datosEntrega.cantidad_entregada / lotesArray.length);
          let cantidadRestante = datosEntrega.cantidad_entregada;
          
          for (let i = 0; i < lotesArray.length; i++) {
            const nombreLote = lotesArray[i];
            ...
            if (stock) {
              const cantidad = i === lotesArray.length - 1 
                ? cantidadRestante // Último lote recibe el resto
                : cantidadPorLote;
      ...
      } else if (calendario.stock_vacuna) {
        // Solo un lote
        lotesEntregados = [{
    ...
    const pdfData = {
      ...
      entrega: {
        ...
        fechaEntrega: datosEntrega.fecha_entrega || new Date(), // Para el template
      ...
      producto: {
        ...
        // NUEVO: Lista de lotes entregados
        lotes_entregados: lotesEntregados,
        tiene_multiples_lotes: lotesEntregados.length > 1
      }
    };
...
exports.verificarEstadoLotes = async (req, res) => {
  ...
  try {
    ...
    for (const calendario of calendarios) {
      ...
      // ✅ NUEVA LÓGICA: Verificar si ya fue entregado completamente
      const dosisEntregadas = calendario.dosis_entregadas || 0;
      const dosisPendientes = calendario.cantidad_dosis - dosisEntregadas;
      
      // Si ya se entregó todo, no verificar nada
      if (dosisEntregadas >= calendario.cantidad_dosis) {
        console.log(`✅ Semana ${calendario.numero_semana} - ${nombreVacuna}: Entrega completada (${dosisEntregadas}/${calendario.cantidad_dosis})`);
        continue; // Saltar al siguiente calendario
      }
      ...
      const problema = {
        ...
        cantidad_pendiente: dosisPendientes,
        dosis_entregadas: dosisEntregadas,
        problemas: []
      };
...
exports.liberarTodosLotes = async (req, res) => {
  try {
    ...
    res.status(400).json({
      success: false,
      message: error.message || 'Error al liberar los lotes'
    });
  }
};
 ...
 * GET /cotizaciones/:id/resumen-lotes
 */
exports.getResumenLotes = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    let itemsSinLote = 0;
    let itemsConLote = 0;
    let itemsAplicados = 0;

    for (const item of cotizacion.calendario_vacunacion) {
      if (item.estado_dosis === 'aplicada') {
        itemsAplicados++;
      }
      ...
      if (lotesMap.has(loteKey)) {
        const loteInfo = lotesMap.get(loteKey);
        loteInfo.cantidad_reservada += item.cantidad_dosis;
        loteInfo.aplicaciones.push({
          id_calendario: item.id_calendario,
          semana: item.numero_semana,
          dia_plan: item.dia_plan,
          fecha_programada: item.fecha_programada,
          estado: item.estado_dosis,
          cantidad_dosis: item.cantidad_dosis
        });
      } else {
        lotesMap.set(loteKey, {
          id_stock_vacuna: item.id_stock_vacuna,
          lote: item.stock_vacuna.lote,
          vacuna_nombre: item.stock_vacuna.vacuna?.nombre || 'Desconocida',
          vacuna_codigo: item.stock_vacuna.vacuna?.codigo || '',
          fecha_vencimiento: item.stock_vacuna.fecha_vencimiento,
          stock_actual: item.stock_vacuna.stock_actual,
          stock_reservado: item.stock_vacuna.stock_reservado,
          cantidad_reservada: item.cantidad_dosis,
          aplicaciones: [{
            id_calendario: item.id_calendario,
            semana: item.numero_semana,
            dia_plan: item.dia_plan,
    ...
    const lotes = Array.from(lotesMap.values()).map(lote => ({
      ...lote,
      puede_liberar: lote.aplicaciones.some(a => a.estado !== 'aplicada')
    }));
    ...
    res.json({
      ...
      data: {
        ...
        estado: cotizacion.estado,
        resumen: {
          total_aplicaciones: cotizacion.calendario_vacunacion.length,
          aplicaciones_con_lote: itemsConLote,
          aplicaciones_sin_lote: itemsSinLote,
          aplicaciones_ya_aplicadas: itemsAplicados,
          lotes_diferentes: lotes.length
        },
        lotes: lotes
      }
    });
